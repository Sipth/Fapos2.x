...
class PagesModel extends FpsModel
{
	...
	public function getEntitiesByHomePage($latest_on_home)
	{
		...
		if (in_array('question', $latest_on_home)) {
			if (!empty($sql)) $sql .= 'UNION ';
			$sql .= "(SELECT `title`, `main`, `date`, `on_home_top`, `id`, `views`, `author_id`, `category_id`, `comments`, (SELECT \"question\") AS skey  FROM `" 
				 . $Register['DB']->getFullTableName('question') . "` "
				 . "WHERE `view_on_home` = '1' AND `available` = '1' AND `premoder` = 'confirmed') ";
		}
		if (in_array('games', $latest_on_home)) {
			if (!empty($sql)) $sql .= 'UNION ';
			$sql .= "(SELECT `title`, `main`, `date`, `on_home_top`, `id`, `views`, `author_id`, `category_id`, `comments`, (SELECT \"games\") AS skey  FROM `" 
